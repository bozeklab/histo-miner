...
- The user is running the code on macOS or Linux, or on Windows via WSL. 
- `histo-miner-env`, `hovernet_submodule` and `mmsegmentation_submodule` conda environments are installed as described in **Installation** section. 
- The `histo-miner` package is installed in the `histo-miner-env` environment (by ssimply running `pip install -e . --no-deps`, see **Installation**).
...
We have to set all the scripts to load the example-configs files instead of the main configs, to create all needed folders, and to download the required data.
...
- SCC Hovernet and SCC Segmenter weights in the `checkpoints` folder
- Scripts loading parameters from the example configs instead of the main config 


**Don't forget to run the `unset_example.py` script once you are fully done with the example. This will
restore loading parameters from the main configs instead of the example one.**
...
1. Run both inferences (it includes pre-processing of the slides):
```bash
cd scripts
...
2. Copy the .json and .png outputs (`scchovernet_output` and `sccsegmenter_output`) inside `post-processing` folder
...
This step is needed if you want to go back using histo-miner from the main folder. Indeed, setting the example changed the pointing of configs to `example/example-configs` instead of the regular use  `configs`. To re-establish the original pointing run:
```bash
conda activate histo-miner-env
cd example
...
Now that you got the features vector for the WSI, you can use it for different downstream analyses, such as WSI classification with interpretability, WSI self-supervised learning, WSI metadata exploration and so on. 
