...
if correction_num == 3:
	...
	for vicijsonfile in jsonfiles:
	    with open(vicijsonfile, 'r') as filename:
	        ...
	        # extract information of the JSON as a string
	        jsonvicinity = filename.read()
	        # read JSON formatted string and convert it to a dict
	        jsonvicinity = json.loads(analysisdata)

	    #find the corresponding distance json file and load it
	    json_path = os.path.split(jsonfile)[0]
	    json_name = os.path.split(jsonfile)[1]
        ...
        with open(distjsonfile, 'r') as filename:
	        ...
	        # extract information of the JSON as a string
	        jsondist = filename.read()
	        # read JSON formatted string and convert it to a dict
	        jsondist = json.loads(analysisdata)


	    jsonvicinity['Distances_of_cells_in_Tumor_Regions'] = jsondist['Distances_of_cells_in_Tumor_Regions']
