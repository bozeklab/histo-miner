...
# Now we now the best number of features for the classification. 
# Previously, for optimization reasons, we did HP search nested cross validation only for all features.
# Now we do it for the given nummber of feature that we know are optimal after cross validation. 
...
run_xgboost = True # because we are in this specific case with this script  
# Like following:
    ...
    for i in range(nbr_of_splits):  
        ...
        for paramset in tqdm(ParameterGrid(xgboost_param_grid)):
            ...
            for l in range(nbr_of_inner_splits):  
                ...
                inner_X_train_reduced = inner_X_train[:, selfeat_mrmr_index_reduced]
                inner_X_val_reduced = inner_X_val[:, selfeat_mrmr_index_reduced]

                #Training
                xgboost_inner_training = xgboost.fit(inner_X_train_reduced, inner_y_train)

                # Predictions on the test split
                y_inner_pred = xgboost_inner_training.predict(inner_X_val_reduced)

                # Calculate balanced accuracy for the current split
                inner_balanced_accuracy = balanced_accuracy_score(inner_y_val, 
                                                                y_inner_pred)

                # Update mrmr list with the all feature evaluation
                balanced_accuracy.append(inner_balanced_accuracy)
        ...
        # Update mrmr list with the all feature evaluation
        balanced_accuracy.append(best_inner_balanced_accuracy)
        ...
        balanced_accuracies['balanced_accuracies_mrmr'][currentsplit] = best_inner_balanced_accuracy
...
for i in range(nbr_of_splits):
    ...
    balanced_accuracy_mrmr = np.asarray(
        balanced_accuracies['balanced_accuracies_mrmr'][currentsplit]
        ) 
...
with open(save_text_path, 'w') as file:
    ...
    file.write('\n\nBest mean balanced accuracy is:' +  
        str(mean_balanced_accuracies_mrmr)) 
